...
public partial class SimpleBRGExample : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        for (int i = 0; i < m_TargetPoints.Length; i++)
        {
            ...
            newTargetPos.x = random.NextFloat(0, radius);
            newTargetPos.z = random.NextFloat(0, radius);
    ...
    private void Update()
    {
        ...
        NativeArray<bool> show = new NativeArray<bool>(Show, Allocator.TempJob);
        ...
        NativeArray<float3> targetMovePointsCarmeraPos = new NativeArray<float3>(m_TargetPoints.Length, Allocator.TempJob); //worldToObject

        for (int i = 0; i < kNumInstances; i++)
        {
            Vector3 viewPos = Camera.main.WorldToViewportPoint(matrices[i].GetPosition());
            targetMovePointsCarmeraPos[i] = viewPos;
        }
        ...
        var moveJob = new RandomMoveJob
        {
            ...
            targetMovePointsCarmeraPos = targetMovePointsCarmeraPos,
            show = show,
        };
    ...
    private void PopulateInstanceDataBuffer()
    {
        ...
        for (int i = 0; i < kNumInstances; i++)
        {
            matrices[i] = Matrix4x4.Translate(Vector3.zero);
...
partial struct RandomMoveJob : IJobParallelFor
{
    ...
    public NativeArray<float3> targetMovePointsCarmeraPos;
