...
using GXGame.Scripts.Runtime.RVO;
using UnityEngine;
    ...
    internal class Agent
    {
        ...
        internal void computeNewVelocity()
        {
            ...
            for (int i = 0; i < obstacleNeighbors_.Count; ++i)
            {
                ...
                {
                    if (RVOMath.det(invTimeHorizonObst * relativePosition1 - orcaLines_[j].point, orcaLines_[j].direction) - invTimeHorizonObst * radius_ >=
                        -RVOMath.RVO_EPSILON &&
                        RVOMath.det(invTimeHorizonObst * relativePosition2 - orcaLines_[j].point, orcaLines_[j].direction) - invTimeHorizonObst * radius_ >=
                        -RVOMath.RVO_EPSILON)
                    {
                        alreadyCovered = true;
                ...
                float s = ((-relativePosition1).X(obstacleVector)) / RVOMath.absSq(obstacleVector);
                ...
                float t = obstacle1 == obstacle2 ? 0.5f : ((velocity_ - leftCutOff).X(cutOffVector)) / RVOMath.absSq(cutOffVector);
                float tLeft = (velocity_ - leftCutOff).X(leftLegDirection);
                float tRight = (velocity_ - rightCutOff).X(rightLegDirection);
            ...
            for (int i = 0; i < agentNeighbors_.Count; ++i)
            {
                ...
                if (distSq > combinedRadiusSq)
                {
                    ...
                    float dotProduct1 = w.X(relativePosition);
                    ...
                    else
                    {
                        ...
                        float dotProduct2 = relativeVelocity.X(line.direction);
        ...
        {
            float dotProduct = lines[lineNo].point.X(lines[lineNo].direction);
            ...
            {
                /* Optimize direction. */
                if (optVelocity.X(lines[lineNo].direction) > 0.0f)
                {
                    /* Take right extreme. */
            ...
            else
            {
                ...
                float t = lines[lineNo].direction.X(optVelocity - lines[lineNo].point);
                ...
                if (RVOMath.det(lines[i].direction, lines[i].point - result) > distance)
                {
                    ...
                    for (int j = numObstLines; j < i; ++j)
                    {
                        ...
                        if (RVOMath.fabs(determinant) <= RVOMath.RVO_EPSILON)
                        {
                            ...
                            if (lines[i].direction.X(lines[j].direction) > 0.0f)
                            {
